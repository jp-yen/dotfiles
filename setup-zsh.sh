...
echo " (MINGW64 / MSYS2 では管理者権限で実行してください)"
echo "=========================================="
...
detect_distribution() {
    ...
    # MSYS2 / MINGW の判定
    if echo "$OS_NAME" | grep -q -E "MINGW|MSYS|CYGWIN"; then
        DISTRO_FAMILY="msys2"
        PKG_MANAGER="pacman"
        DISTRO_NAME="$OS_NAME"
        DISTRO_ID="msys2"
        ...
        echo "エラー: OSの情報を特定できません"
        exit 1
    fi

    echo "検出された環境: $DISTRO_NAME ($DISTRO_ID)"
...
# /etc/zsh/zshrc ローダーの設定
configure_zshrc_loader() {
    echo "システム設定ローダーを設定しています..."

    local ZSHRC_SYS_PATH="/etc/zsh/zshrc"
...
EOF
        chmod 644 "$ZSHRC_SYS_PATH"
        echo "✓ $ZSHRC_SYS_PATH を更新しました"
    fi
}
...
create_system_config() {
    ...
    if [ ! -d "$SOURCE_DIR" ]; then
        echo "警告: 設定ファイルディレクトリが見つかりません: $SOURCE_DIR (スキップします)"
    else
        mkdir -p "$CONFIG_DIR"
    ...
    if [ "$PKG_MANAGER" = "dnf" ]; then
        configure_fedora_zshrc
    elif [ "$PKG_MANAGER" = "apt" ] || [ "$PKG_MANAGER" = "apk" ] || [ "$PKG_MANAGER" = "pacman" ]; then
        configure_zshrc_loader
    fi
...
install_helper_functions() {
    ...
    if [ ! -d "$SOURCE_BIN" ]; then
        echo "警告: バイナリディレクトリが見つかりません: $SOURCE_BIN (スキップします)"
        return
    fi
